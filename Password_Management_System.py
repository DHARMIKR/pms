...
def batch_password_generation():
    ...
    if batch_number >= 100:
        return {"message": "You cannot generate more than 100 passwords"}
    else:
        password_list = []
        for _ in range(batch_number):
            while True:
                try:
                    password = Pass_Rust.random_password_generator(14)
                    break
                except ValueError:
                    continue
...
def secure_password_checker(password=None, token=None):
    ...
    if password_breach_checking:
        ...
        return {'message': Pass_Rust.checking_password_security(password)}
